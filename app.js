...
(function initStars() {
  ...
  setInterval(() => { spawnMeteor(); spawnMeteor(); }, 3000);
...
// ─────────────────────────────────────────
// 5. PENDIENTES — guardados en localStorage
//    (solo en el ordenador del usuario)
// ─────────────────────────────────────────
const PENDIENTES_KEY = 'pendientesLocales';

function getPendientesLocales() {
  try { return JSON.parse(localStorage.getItem(PENDIENTES_KEY) || '[]'); } catch { return []; }
}

function savePendientesLocales(lista) {
  localStorage.setItem(PENDIENTES_KEY, JSON.stringify(lista));
}

function loadPendientes() {
  const lista = getPendientesLocales();
  lista.forEach(p => appendPendienteDOM(p.id, p.texto));
}
...
function addPendiente() {
  ...
  const lista = getPendientesLocales();
  lista.push({ id, texto });
  savePendientesLocales(lista);
  appendPendienteDOM(id, texto);
  input.value = '';
}

function deletePendiente(id, btn) {
  const lista = getPendientesLocales().filter(p => p.id !== id);
  savePendientesLocales(lista);
...
// ─────────────────────────────────────────
// 10. CARDS ESTÁTICAS — ya no existen en el HTML.
//     Las funciones se mantienen vacías por si
//     quedara alguna referencia residual.
// ─────────────────────────────────────────
function editStaticCard() {}
function deleteStaticCard() {}
...
async function saveCard() {
  ...
  // Card estática — solo DOM + localStorage
  if (!editId && modal._staticCardEl) {
    const card    = modal._staticCardEl;
    const idx     = card.dataset.staticIndex;
    const titleEl = card.querySelector('.card-title');
    const descEl  = card.querySelector('.card-desc');
    titleEl.textContent = title;
    titleEl.className   = `card-title ${color}-text`;
    descEl.textContent  = desc;
    ['purple','blue','teal','amber','pink','green'].forEach(c => card.classList.remove(c));
    card.classList.add(color);
    card.setAttribute('onclick', `openLink('${url}')`);
    if (idx !== undefined) saveStaticOverride(idx, { title, desc, url, color });
